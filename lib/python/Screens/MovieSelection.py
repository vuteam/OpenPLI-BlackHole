...
l_moviesort = [(str(MovieList.SORT_GROUPWISE), _('default'), '02/01 & A-Z'),
...
class MovieBrowserConfiguration(ConfigListScreen, Screen):
    ...
    def __init__(self, session, args = 0):
        ...
        configList = [getConfigListEntry(_('Sort'), cfg.moviesort),
        ...
        self['setupActions'] = ActionMap(['SetupActions', 'ColorActions', 'MenuActions'], {'red': self.cancel,
         'green': self.save,
         'save': self.save,
         'cancel': self.cancel,
         'ok': self.save,
...
class MovieSelection(Screen, HelpableScreen, SelectionEventInfo, InfoBarBase, ProtectedScreen):
    ...
    def __init__(self, session, selectedmovie = None, timeshiftEnabled = False):
        ...
        self.settings = {'listtype': config.movielist.listtype.value,
        ...
        self['SeekActions'] = HelpableActionMap(self, 'InfobarSeekActions', {'playpauseService': (self.preview, _('Preview')),
         'seekFwd': (sfwd, tFwd),
    ...
    def loadLocalSettings(self):
        if config.movielist.settings_per_directory.value:
            try:
                ...
                updates = {'listtype': config.movielist.listtype.default,
            ...
            updates = {'listtype': config.movielist.listtype.value,
             'moviesort': config.movielist.moviesort.value,
             'description': config.movielist.description.value,
             'movieoff': config.usage.on_movie_eof.value}
            self.applyConfigSettings(updates)
    ...
    def do_createdir(self):
        ...
        self.session.openWithCallback(self.createDirCallback, VirtualKeyBoard, title=_('Please enter name of the new directory'), text='')
    ...
    def do_sort(self):
        ...
        self.sorttimer.start(1500, True)
